...
do
    echo "=============================="
    echo "Running reschedule overhead experiment (${estimations[$i]})"
    echo "=============================="
    mkdir -p logs/7_4_logs/device_${estimations[$i]}_logs
    rm -f logs/7_4_logs/device_${estimations[$i]}_logs/*.log
    ...
    rm -f ../logs/7_4_logs/home_assistant_${estimations[$i]}.log
    mkdir -p ./config_tmp
...
# cp ../rasc_configs/rasc_7_4_scalability_10.yaml ./config_tmp/rasc.yaml
