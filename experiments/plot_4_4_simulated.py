...
def main():
    ...
    for i, level in enumerate(levels):
        with open(f"experiments/interruption_{level}.json", "r", encoding="utf-8") as f:
            result = json.load(f)
        # plot_4_4(result, axs[0][i], level, i == 0)
        # plot_detection_time(result, axs[1][i], level, i == 0)
        plot_failed_rate(result, axs[i], level, i == 0, i == 1)

    fig.subplots_adjust(wspace=0.05, hspace=0)
    fig.savefig("results/interruption.pdf", bbox_inches="tight")
