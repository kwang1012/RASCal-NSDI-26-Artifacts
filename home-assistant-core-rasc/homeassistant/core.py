...
class HomeAssistant:
    ...
    def async_add_hass_job(
        ...
        if hassjob.job_type == HassJobType.Coroutinefunction:
            ...
            task = self.loop.create_task(
                hassjob.target(*args), name=hassjob.name)
    ...
    async def _await_and_log_pending(
        ...
        while pending:
            ...
            for task in pending:
                _LOGGER.debug("Waited %s seconds for task: %s",
                              wait_time, task)
    ...
    async def async_stop(self, exit_code: int = 0, *, force: bool = False) -> None:
        ...
        for task in list(running_tasks):
            ...
            try:
                ...
                _LOGGER.exception(
                    "Task %s error during stage 3 shutdown: %s", task, ex)
...
class EventBus:
    ...
    def async_listen_once(
        ...
        functools.update_wrapper(
            _onetime_listener, listener, ("__name__",
                                          "__qualname__", "__module__"), []
        )

        filterable_job = (
            HassJob(_onetime_listener,
...
class State:
    ...
    def from_dict(cls, json_dict: dict[str, Any]) -> Self | None:
        ...
        if context := json_dict.get("context"):
            context = Context(id=context.get(
                "id"), user_id=context.get("user_id"))
...
class States(UserDict[str, State]):
    ...
    def __init__(self) -> None:
        ...
        super().__init__()
        self._domain_index: defaultdict[str,
                                        dict[str, State]] = defaultdict(dict)
...
class ServiceRegistry:
    ...
    async def async_call(
        ...
        if DOMAIN_RASC in self._hass.data:
            rasc: RASCAbstraction = self._hass.data[DOMAIN_RASC]
            a_coro, s_coro, c_coro = rasc.execute_service(
...
class Config:
    ...
    def distance(self, lat: float, lon: float) -> float | None:
        ...
        return self.units.length(
            location.distance(self.latitude, self.longitude,
                              lat, lon), LENGTH_METERS
        )
