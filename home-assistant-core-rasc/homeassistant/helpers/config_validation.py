...
def is_regex(value: Any) -> re.Pattern[Any]:
    ...
    try:
        ...
        raise vol.Invalid(
            f"value {value} is not a valid regular expression") from err
...
def entity_domain(domain: str | list[str]) -> Callable[[Any], str]:
    ...
    def validate(value: str) -> str:
        ...
        if len(validated) != 1:
            raise vol.Invalid(
                f"Expected exactly 1 entity, got {len(validated)}")
        return validated[0]
...
_TIME_PERIOD_DICT_KEYS = ("days", "hours", "minutes",
...
time_period = vol.Any(time_period_str, time_period_seconds,
                      timedelta, time_period_dict)
...
def _positive_time_period_template_complex(value: Any) -> Any:
    ...
    for key, element in value.items():
        ...
        if not isinstance(element, str) or (
            isinstance(element, str) and not template_helper.is_template_string(
                element)
        ):
    ...
    def validator(config: dict) -> dict:
        ...
        if replacement_key:
            ...
            if value is not None and (
                replacement_key not in config or default == config.get(
                    replacement_key)
            ):
...
def key_value_schemas(
    ...
    def key_value_validator(value: Any) -> dict[Hashable, Any]:
        ...
        alternatives = ", ".join(str(alternative)
                                 for alternative in value_schemas)
...
ENTITY_SERVICE_FIELDS = {
    ...
    vol.Optional(ATTR_DEVICE_ID): vol.Any(
        ENTITY_MATCH_NONE, vol.All(
            ensure_list, [vol.Any(dynamic_template, str)])
    ),
...
TARGET_SERVICE_FIELDS = {
    ...
    # complex template, handling it like this, keeps config validation useful.
    vol.Optional(ATTR_ENTITY_ID): vol.Any(
        comp_entity_ids_or_uuids, dynamic_template, vol.All(
            list, template_complex)
    ),
    vol.Optional(ATTR_DEVICE_ID): vol.Any(
        ENTITY_MATCH_NONE, vol.All(
            ensure_list, [vol.Any(dynamic_template, str)])
...
NUMERIC_STATE_THRESHOLD_SCHEMA = vol.Any(
    vol.Coerce(float), vol.All(str, entity_domain(
...
_SCRIPT_REPEAT_SCHEMA = vol.Schema(
    {
        **SCRIPT_ACTION_BASE_SCHEMA,
        vol.Required(CONF_REPEAT): vol.All(
            ...
            has_at_least_one_key(CONF_COUNT, CONF_FOR_EACH,
                                 CONF_WHILE, CONF_UNTIL),
        ),
    }
)
