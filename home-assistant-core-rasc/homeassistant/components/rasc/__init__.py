...
def run_experiments(hass: HomeAssistant, rasc: RASCAbstraction):
    ...
    async def wrapper(_):
        ...
        for setting in settings:
            ...
            # key = f"{entity_id},{action}"
            # if action == SERVICE_SET_TEMPERATURE:
            #     if RASC_THERMOSTAT not in setting:
            #         raise ValueError("Thermostat setting not found")
            #     key += f",{setting[RASC_THERMOSTAT][RASC_THERMOSTAT_START]},{setting[RASC_THERMOSTAT][RASC_THERMOSTAT_TARGET]}"
            # 4.1
            for _ in range(10):
                LOGGER.info("Start action %s on %s with data %s", action, entity_id, service_data)
                a_coro, s_coro, c_coro = hass.services.rasc_call(device, action, {"entity_id": entity_id, **service_data})
                await a_coro
                await s_coro
                await c_coro
                LOGGER.info("Completed action %s on %s", action, entity_id)

                LOGGER.info("Start reset action %s on %s with data %s", reset_action, entity_id, reset_service_data)
                a_coro, s_coro, c_coro = hass.services.rasc_call(device, reset_action, {"entity_id": entity_id, **reset_service_data})
                await a_coro
                await s_coro
                await c_coro
                LOGGER.info("Completed reset action %s on %s", reset_action, entity_id)
            # interruption
    ...
    try:
        for service_call in service_calls:
            ...
            _, _, c_coro = hass.services.rasc_call(
                domain, service, {
                    "entity_id": entity_id, "params": service_call}
            )
            await c_coro
        return True
    except ServiceFailureError as e:
        LOGGER.error(
            f"Failed to initialize entity state: {entity_id}, service: {service}, error: {e}"
        )
        return False
...
def examine_final_state(hass: HomeAssistant, config: ConfigType):
    ...
    for entity_id in routine_setup_conf:
        ...
        if state.domain == "light":
            final_states[entity_id]["brightness"] = state.attributes.get(
                "brightness", None)
    ...
    dataset = config["rasc"][CONF_ROUTINE_ARRIVAL_FILENAME].split(".")[
        0].split("_")[1]
    if "debug" in dataset:
        return
