...
class BaseRescheduler(TimeLineScheduler):
    ...
    async def _move_device_schedule(
        ...
        for action_id, action_lock in self._lineage_table.lock_queues[
            ...
            if action_lock.action.start_requested:
                ...
                continue
            action_length = action_lock.action.length(
                entity_id) or timedelta(0)
        ...
        while next_action_heap:
            ...
            for entity_id in entities:
                if entity_id not in free_slots:
                    raise ValueError(
                        "Entity %s has no free slots." % entity_id)
                if (
                ...
                self.schedule_action(
                    free_slot, (action_st,
                                action_end), free_slots[entity_id], entity_id
                )
    ...
    def affected_actions_after_len_diff(
        ...
        for routine_after_id in routines_after:
            routine_after_actions = self._routine_actions_after(
    ...
    def get_immutable_serialization_order(self, now: datetime) -> list[str]:
        ...
        while postsets:
            ...
            if not last:
                raise ValueError(
                    "There is a cycle in the serialization order.")
            ...
            routine_ranks: dict[str, float] = self._routine_ranks(
                routine_ids=last)
            if self._routine_prioriy_policy in (EARLIEST, SHORTEST):
                routine_order = dict(
                    sorted(routine_ranks.items(), key=lambda x: x[1]))
            elif self._routine_prioriy_policy in (LATEST, LONGEST):
                routine_order = dict(
                    sorted(routine_ranks.items(),
    ...
    def deschedule_affected_actions(
        ...
        LOGGER.debug(f"{affected_action_ids=}")
    ...
    def _find_action_start_after(
        ...
        action_length = max(action.length(entity_id)
    ...
    def reschedule_all_action(
        ...
        for target_entity in target_entities:
            ...
            self.schedule_lock(
                action, (action_st, action_end), entity_id, lock_queues)
    ...
    def _output_wait_queues(
        ...
        for _entity_id, wait_queue in wait_queues.items():
            ...
            LOGGER.debug("Entity %s's wait queue: %s",
                         _entity_id, wait_queue_str)
    ...
    def sjf(  # noqa: C901
        ...
        for action in descheduled_actions.values():
            ...
            for target_entity in target_entities:
                ...
                if entity_id not in next_slots:
                    last_slot_start = self._lineage_table.free_slots[entity_id].end()[
                        0]
        ...
        LOGGER.debug("Descheduled actions: %s",
                     list(descheduled_actions.keys()))
        while descheduled_actions:
            ...
            if serializability_guarantee:
                shortest_action_routine_id = get_routine_id(
            ...
            for child in shortest_action.all_children:
                ...
                for target_entity in target_entities:
                    entity_id = get_entity_id_from_number(
                        self._hass, target_entity)
                    if entity_id not in wait_queues:
                        wait_queues[entity_id] = list[tuple[timedelta,
                                                            ActionEntity]]()
                    ...
                    # )
                    heapq.heappush(
                        wait_queues[entity_id], (child.length(
                            entity_id), child)
                    )
                    # LOGGER.debug(f"Added {child.action_id} to {entity_id}'s wait queue")
            ...
            target_entities = get_target_entities(
                self._hass, shortest_action.action)
            to_remove = set[str]()
            for entity_id in target_entities:
                # update the schedule metrics
    ...
    def _optimal_helper(  # noqa: C901
        ...
        for child in chosen_action.all_children:
            ...
            for target_entity in target_entities:
                ...
                if entity_id not in next_slots:
                    last_slot_start = lineage_table.free_slots[entity_id].end()[
                        0]
                    if not last_slot_start:
                        continue
                    next_slots[entity_id] = last_slot_start
        ...
        # filter out entities with no actions to schedule
        filtered_next_slots = {k: v for k,
                               v in next_slots.items() if k in wait_queues}
    ...
    def optimal(
        ...
        if not best_lt or best_so is None or best_metric is None:
            return self._lineage_table, self._serialization_order
        LOGGER.debug(
            "Optimal schedule metric: %s", best_metric.get(
                self._optimal_sched_metric)
        )
        ...
        while next_batch:
            ...
            for action in candidates:
                ...
                for target_entity in target_entities:
                    ...
                    if entity_id not in self._lineage_table.lock_queues:
                        raise ValueError(
                            "Entity %s has no schedule." % entity_id)
        ...
        for dependency, parent_set in action.parents.items():
            for parent_action in parent_set:
                ...
                if not parent_action_id:
                    raise ValueError(
                        "The parent action does not have an action ID.")
                target_entities = get_target_entities(
    ...
    def _action_start_time(self, action: ActionEntity) -> datetime:
        ...
        for target_entity in target_entities:
            ...
            earliest_action_start = min(
                earliest_action_start, action_lock.start_time)
        if not earliest_action_start:
            raise ValueError("Action %s has not been scheduled." %
                             action.action_id)
    ...
    async def displace_action_to_idle_time(  # unused so far  # noqa: C901
        ...
        for routine_id, source_actions in routines_after_actions.items():
            current_routine_action_set = self._bfs_actions(
                list(source_actions))
                ...
                for action in current_routine_action_set:
                    ...
                    for target_entity_id in target_entities:
                        ...
                        end_time_per_entity[target_entity_id] = max(
                            end_time_per_entity.get(
                                target_entity_id, action_end),
                            action_end,
                        )
                continue
            ...
            if not routine:
                raise ValueError(
                    "Routine %s has not been scheduled." % routine_id)
        ...
        for action in next_independent_actions:
            if not action.action_id:
                raise ValueError(
                    "An independent action does not have an action ID.")
            target_entities = get_target_entities(self._hass, action.action)
            entity_ids = [
                get_entity_id_from_number(self._hass, target_entity)
                for target_entity in target_entities
    ...
    async def _fill_slot(
        ...
        free_slots = self._lineage_table.free_slots
        self.reschedule_all_action(
            action, new_action_start, free_slots, lock_queues)
        return True
...
class RascalRescheduler:
    ...
    async def _reschedule(
        ...
        print("Serialization order:",
              "->".join(key[-5:] for key in old_so.keys()))
            ...
            if diff.total_seconds() < 0:
                ...
                for slot_st, slot_end in free_slots.items():
                    ...
                    if slot_st == old_end_time:
                        free_slots.insert_before(
                            slot_st, new_end_time, slot_end)
                        free_slots.pop(slot_st)
                        found = True
                        break
                if not found:
        ...
        if self._resched_policy in (RV):
            ...
            if not affected_actions:
                print("\033[92m No affected actions\033[00m")
                LOGGER.debug(
            ...
            print("Affected actions:", ", ".join(sorted(
                f"{aid.split('.')[0][-5:]}.{aid.split('.')[-1]}" for aid in affected_actions.keys())))
    ...
    async def _handle_overtime(self, event: Event, diff: float) -> None:
        ...
        entity_id: Optional[str] = event.data.get(ATTR_ENTITY_ID)
        action_id: Optional[str] = event.data.get(ATTR_ACTION_ID)
        context_id: Optional[str] = event.data.get(ATTR_CONTEXT_ID)
        event_type: Optional[str] = event.data.get(CONF_TYPE)
        if not action_id:
        ...
        if self._resched_trigger in (ANTICIPATORY, REACTIVE):
            ...
            extra = self._get_extra_proactive(
                action, entity_id, context_id, expected_action_length
            )
        if extra <= 0:
            if saved_cancel:
                saved_cancel()
    ...
    def setup_overtime_check(self, event: Event, timer_delay: timedelta) -> None:
        ...
        if timer_delay <= 0:
            LOGGER.debug(
                "Timer delay is less than or equal to 0, not setting up timer")
            return
    ...
    def _cancel_overtime_check(self, event: Event) -> None:
        ...
        if saved_cancel:
            saved_cancel()
            self._timer_handles[entity_id] = (None, None)
        LOGGER.debug("Canceled overtime check for %s on %s",
                     action_id, entity_id)
