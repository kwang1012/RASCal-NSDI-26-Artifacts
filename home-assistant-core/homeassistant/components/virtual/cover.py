...
async def async_setup_entry(
    ...
    for entity_config in get_entity_configs(
        ...
        if entity_config[CONF_COORDINATED]:
            entity = cast(
                VirtualCover, CoordinatedVirtualCover(
                    entity_config, coordinator)
            )
        else:
...
class VirtualCover(VirtualEntity, CoverEntity):
    ...
    def __init__(self, config) -> None:
        ...
        self._attr_device_class = config.get(
            CONF_CLASS, CoverDeviceClass.SHADE)
    ...
    def _create_state(self, config):
        ...
        self._attr_is_closed = config.get(
            CONF_INITIAL_VALUE).lower() == STATE_CLOSED
    ...
    async def _start_operation(self):
        ...
        try:
            ...
            step = (target_position -
                    self._attr_current_cover_position) / action_length
            while True:
    ...
    async def async_stop_cover(self, delay):
