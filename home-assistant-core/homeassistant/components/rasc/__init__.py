        ...
        if RASC_DETECTION_TIME_EXPS in rasc.config:
            ...
            for setting in settings:
                ...
                for _ in range(10):
                    ...
                    LOGGER.info("Start reset action %s on %s with data %s",
                                reset_action, entity_id, reset_service_data)
                    a_coro, s_coro, c_coro = hass.services.rasc_call(
                        device, reset_action, {"entity_id": entity_id, **reset_service_data})
                    await a_coro
                    await s_coro
                    await c_coro
                    LOGGER.info("Completed reset action %s on %s",
                                reset_action, entity_id)
