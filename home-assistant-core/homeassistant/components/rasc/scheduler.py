...
def _create_routine(  # noqa: C901
    ...
    if CONF_PARALLEL in script:
        ...
        if domain == DOMAIN_SCRIPT:
            ...
            if script_component:
                base_script = script_component.get_entity(
                    list(script.values())[0])
...
def get_target_entities(hass: HomeAssistant, script: dict[str, Any]) -> list[str]:
    ...
    if CONF_SERVICE in script:
        ...
        try:
            target_entities = [get_entity_id_from_number(
                hass, script[CONF_ENTITY_ID])]
        except Exception as _:  # pylint: disable=broad-except
            _LOGGER.error("%s not found in the script: %s",
...
def output_free_slots(
    ...
    for entity_id, timeline in timelines.items():
        ...
        for st, end in timeline.items():
            ...
            sub_entity_json: dict[str, str | None] = {
                "st": st_str, "end": end_str}
...
class LineageTable:
    ...
    def consistent_free_slots(self) -> dict[str, Queue[datetime, datetime]]:
        ...
        current_time = self.last_removed_time or datetime.now(timezone.utc)
        free_slots = {}
        for entity_id, lock_queue in self._lock_queues.items():
    ...
    def visualize(self, granularity=timedelta(seconds=0.5)) -> str:
        ...
        min_time = min(action_info.start_time.astimezone(timezone.utc) for queue in filtered_lock_queues.values()
        ...
        lines = [{key: [" " * slot_width for _ in range(slots_per_line)] for key in filtered_lock_queues}]
        for entity_id, lock_queue in filtered_lock_queues.items():
            ...
            for action_id, action_info in lock_queue.items():
                action_print_id = f"{get_routine_id(action_id)[-5:]}.{action_id.split('.')[-1]}"
                start_idx_overall = int(
                    (action_info.start_time.astimezone(timezone.utc) - min_time) / granularity)
                end_idx_overall = int(
                    (action_info.end_time.astimezone(timezone.utc) - min_time) / granularity)
    ...
    def add_entity(self, entity_id: str) -> None:
        ...
        self._free_slots[entity_id] = Queue[datetime,
                                            datetime]({datetime.now(timezone.utc): None})
    ...
    def delete_entity(self, entity_id: str) -> None:
        ...
        try:
            ...
            raise KeyError(
                f"While deleting entity {entity_id} in lineage table") from e
...
class BaseScheduler:
    ...
    def get_first_action_with_acquired_lock(self, entity_id: str) -> ActionInfo | None:
        ...
        lock_queue: Queue[str,
                          ActionInfo] = self._lineage_table.lock_queues[entity_id]
    ...
    def get_ts_by_nolease(
        ...
        action: ActionInfo | None = self.get_first_action_with_acquired_lock(
            entity_id)
    ...
    def get_ts_by_prelease(
        ...
        slot_start = self.find_ts_before_action(
            action, free_slots[entity_id], now)
        slot_end = free_slots[entity_id].get(
            slot_start) if slot_start else None
        ...
        if self.action_running(action, entity_id, "pre"):
            _LOGGER.warning(
                "Failed to prelease the lock. The action is running")
            return None
    ...
    def find_ts_before_action(
        ...
        return next(
            (st for st, end in free_slots.items()
             if end == action_st and end > now),
            None,
        )
    ...
    def get_ts_by_postlease(
        ...
        action: ActionInfo | None = self.get_last_action_with_acquired_lock(
            entity_id)
    ...
    def conflict_determined_serializability(
        ...
        idx1 = self._serialization_order.index(
    ...
    def action_running(
        ...
        action_info = self._lineage_table.lock_queues[entity_id].get(
            action.action_id)
    ...
    def schedule_lock(
        ...
        for action_id, action_info in lock_queues[entity_id].items():
            ...
            if new_action_slot[1] <= action_info.start_time:
                ...
                self.update_real_schedule(
                    entity_id, new_action, new_action_slot)
                _LOGGER.debug(
    ...
    def schedule_routine(
        ...
        next_end_time: datetime | None = datetime.now(timezone.utc)
...
class FirstComeFirstServeScheduler(BaseScheduler):
    ...
    def __init__(
        ...
        super().__init__(hass, lineage_table, serialization_order, scheduling_policy)
    ...
    def conflict_determined_serializability_in_case_tl(
        ...
        for routine_in_preset in preset:
            ...
            for routine_in_poset in postset:
                ...
                if idx1 > idx2:
                    _LOGGER.warning(
                        "Conflict determined serializability in timeline")
                    return True
    ...
    def _schedule_routine_in_case_tl(
        ...
        if CONF_PARALLEL in script:
            for item in list(script.values())[0]:
                ...
                next_end_time = {
                    entity_id: max(
                        next_end_time[entity_id], item_end_time[entity_id])
...
class RascalScheduler(BaseScheduler):
    ...
    def set_reschedule_handler(
        self, handler: Callable[[Event], None] # Coroutine[Any, Any, None]]
    ) -> None:
        ...
        if self._reschedule_handler is not None:
            logging.warning(
                "Reschedule handler is already set. ")
            return
    ...
    def _add_routine_to_serialization_order(
        ...
        _LOGGER.debug("Add routine %s to the serialization order",
                      routine.routine_id)
        output_all(_LOGGER, serialization_order=self._serialization_order)
    ...
    def _remove_routine_from_serialization_order(self, routine_id: str) -> None:
        ...
        if routine_id in self._serialization_order:
            _LOGGER.debug(
                "Remove routine %s from the serialization order", routine_id)
        ...
        for action in list(routine.actions.values())[:-1]:
            ...
            for entity in target_entities:
                ...
                if action.action_id is not None:
                    self._lineage_table.lock_queues[entity_id].pop(
                        action.action_id)
    ...
    def _attempt_lock(
        ...
        _LOGGER.error("Action %s failed to acquired the lock %s",
                      action_id, entity_id)
        return False
    ...
    def _acquire_lock(self, action_id: str, entity_id: str) -> None:
        ...
        self._update_action_lock_state(
            action_id, entity_id, LOCK_STATE_ACQUIRED)
    ...
    def _release_routine_locks(self, routine: RoutineEntity) -> None:
        ...
        _LOGGER.debug("Release all locks for the routine %s",
                      routine.routine_id)
    ...
    def _remove_scheduled_actions(self, routine_id: str) -> None:
        ...
        _LOGGER.debug(
            "Remove all scheduled actions of the routine %s", routine_id)

    def _eligibility_test(self, routine: RoutineEntity) -> bool:
        """Eligibility test for the routine."""

        _LOGGER.debug("Start eligibility test for the routine %s",
                      routine.routine_id)
        ...
        if self._scheduling_policy in (FCFS, FCFS_POST, JIT):
            if isinstance(self._scheduler, TimeLineScheduler):
                raise TypeError(
                    "The scheduler should not be TimeLineScheduler")
        ...
        routine_info.pass_eligibility = True
        _LOGGER.debug("Routine %s pass the eligibility test",
                        routine.routine_id)
        ...
        _LOGGER.debug("Finish eligibility test for the routine %s",
                        routine.routine_id)
        return True
    ...
    def initialize_routine(self, routine: RoutineEntity) -> None:
        ...
        self._metrics.record_routine_arrival(
            routine.routine_id, datetime.now(timezone.utc), sink_actions
        )
    ...
    async def _start_action(self, action: ActionEntity) -> None:
        ...
        if not target_entities:
            raise ValueError(
                f"Action {action.action_id} has no target entities.")
        ...
        while not self._is_action_ready(action):
            ...
            if random_entity_id not in self._lineage_table.lock_queues:
                raise ValueError("Entity %s has no schedule." %
                                 random_entity_id)
            ...
            if not self._are_prev_actions_over(action):
                ...
                for entity in target_entities:
                    ...
                    event = Event("", event_data, time_fired=datetime.now(timezone.utc))
                    if self._reschedule_handler:
                        # await self._reschedule_handler(event)
                        self._reschedule_handler(event)
    ...
    def _attempt_start_action(self, action: ActionEntity) -> None:
        ...
        if not self._is_action_ready(action):
            _LOGGER.debug("Action %s is not ready to start",
                            action.action_id)
            return
    ...
    def _is_action_ready(self, action: ActionEntity) -> bool:
        ...
        if self._scheduling_policy in (FCFS, FCFS_POST, JIT):
            ...
            target_entities = get_target_entities(self._hass, action.action)
            if any(
                self._lineage_table.locks[get_entity_id_from_number(
                    self._hass, entity)]
                != get_routine_id(action.action_id)
                for entity in target_entities
            ):
            ...
            for entity in target_entities:
                ...
                if action_lock.start_time > datetime.now(timezone.utc):
                    _LOGGER.warning(
        ...
        # timeline scheduler
        routine_info = self._serialization_order[get_routine_id(
            action.action_id)]
        if not routine_info:
            raise ValueError(
                "Routine %s is not found in the serialization order"
        ...
        async with self.handle_event_lock:
            ...
            if event_type == RASC_COMPLETE:
                self._metrics.record_action_end(
                    datetime.now(timezone.utc), entity_id, action_id)

            if self._reschedule_handler is not None:
                self.remove_time_slots_before_now(
                    datetime.now(timezone.utc), self._lineage_table.free_slots
                )
                # await self._reschedule_handler(event)
                self._reschedule_handler(event)
            ...
            if event_type == RASC_ACK:
                ...
                _LOGGER.debug("Action %s on entity %s is started",
                              action_id, entity_id)
                ...
                output_all(
                    _LOGGER, lock_queues=self._lineage_table.lock_queues)
    ...
    def _return_lock(self, action_id: str, entity_id: str) -> bool:
        ...
        self._postlease_lock(action_id, entity_id,
    ...
    def _set_action_state(self, action_id: str, state: str) -> None:
        ...
        if not action:
            raise ValueError(
                "Action %s is not in the routine script." % action_id)
        if state == RASC_ACK:
    ...
    async def _async_wait_until_beginning(self, action_id: str) -> None:
        ...
        wait_seconds = (action_start - datetime.now(timezone.utc)).total_seconds()
        _LOGGER.debug("Start the action %s %.2fs later",
    ...
    async def _async_wait_until(self, action_id: str, entity_id: str) -> None:
        ...
        wait_seconds = (action_end - datetime.now(timezone.utc)).total_seconds()
    ...
    def _are_parent_deps_satisfied(self, action: ActionEntity) -> bool:
        ...
        for dependency, parent_set in action.parents.items():
            if dependency == RASC_ACK:
                ...
                satisfied = all(
                    parent.action_completed for parent in parent_set)
            else:
    ...
    def _handle_end_of_routine(self, routine_id: str) -> None:
        ...
        _LOGGER.debug("This is the end of routine '%s' (%s)",
    ...
    def _start_ready_routines_jit(self, entity_id: str) -> None:
        ...
        for next_routine_id in self._lock_waitlist[entity_id]:
            ...
            if self._eligibility_test(next_routine):
                _LOGGER.debug(
                    "Routine %s passes the eligibility test", next_routine_id)
                output_all(
                    _LOGGER,
                    locks=self._lineage_table.locks,
                    lock_queues=self._lineage_table.lock_queues,
